...
async def lifespan(app: FastAPI):
    ...
    global ser
    try:
        # 1. Increased baudrate to 115200 for faster buffer clearing
        # 2. write_timeout=0 makes write operations non-blocking (prevents the crash)
        # 3. Disabled hardware flow control (rtscts/dsrdtr)
        ser = serial.Serial(
            port='COM8', 
            baudrate=115200, 
            timeout=0.1, 
        ...
        time.sleep(2) 
        print("Serial connected successfully on COM8")
    ...
    if ser and ser.is_open:
        try:
            # Reset servos to 0 before closing
            data = "0 0\n"
            ser.write(data.encode('utf-8'))
            ser.flush() # Ensure the last command actually leaves the buffer
        except:
            pass
        ser.close()
...
def start_automatic():
    ...
    while True:
        ...
        Z_D = int(final_coordinates[2])
        
        data = f"{pan_off+final_pan:.0f} {tilt_off+90+final_tilt:.0f}\n"
